...
module.exports = async (req, res, next) => {
  ...
  try {
    ...
    console.log(`Team ${teamId} failed to be found at delete-team`);
    return next(err);
  }
  ...
  if (team.events && team.events.length > 0) {
    ...
    try {
      ...
      console.log("A team event failed to be found at delete-team");
      return next(err);
    }
    ...
    for (const event of teamEvents) {
      ...
      if (endDate.isBefore(endOfToday)) {
        ...
        try {
          ...
          console.log(`Event ${event.id} failed to be updated at delete-team`);
          return next(err);
        }
      } else {
        archiveTeam = true;
  ...
  try {
    ...
    console.log("A team member failed to be found at delete-team");
    return next(err);
  }
  ...
  for (const member of teamMembers) {
    ...
    try {
      ...
      console.log(`Member ${member.id} failed to be updated at delete-team`);
      return next(err);
    }
  }
  ...
  if (archiveTeam) {
    ...
    try {
      ...
      console.log(`Team ${team.id} failed to be updated at delete-team`);
      return next(err);
    }
  } else {
    try {
      await Team.deleteOne({ _id: teamId, isArchived: false });
    } catch (err) {
      console.log(`Team ${team.id} failed to be removed at delete-team`);
      return next(err);
    }
  }
