...
module.exports = async (req, res, next) => {
  ...
  const { fundraisingGoal } = req.body;
  if (
    typeof fundraisingGoal !== 'number' ||
    !Number.isInteger(fundraisingGoal)
  ) {
    return res.status(400).json({ fundraisingGoal: 'Should be an integer' });
  }
  if (fundraisingGoal < 1 || fundraisingGoal > 1000000) {
    return res.status(400).json({
      fundraisingGoal: 'Should be between 1 and 1000000'
    });
  }
