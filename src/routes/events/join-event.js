...
module.exports = async (req, res, next) => {
  ...
  try {
    ...
    logger.error(`Event ${eventId} failed to be found at join-event`)
    return next(err)
  }
  ...
  if (event.isOpen) {
    ...
    try {
      ...
      logger.error(`User ${req.user.id} failed to be updated at join-event`)
      return next(err)
    }
    ...
    try {
      ...
      logger.error(`Event ${event.id} failed to be updated at join-event`)
      return next(err)
    }
    ...
    try {
      ...
      logger.error(
        `Petition from user ${req.user
          .id} to event ${event.id} failed to be found at join-event`
      )
      return next(err)
    }
