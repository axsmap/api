...
module.exports = async (req, res, next) => {
  ...
  const submittedInviteCode = req.body && req.body.inviteCode;
  if (event.isInviteOnly && submittedInviteCode !== event.inviteCode) {
    return res.status(403).json({
      general:
        'This mapathon is invite-only. Use the organizer invite link to join.'
    });
  }

  const endDate = moment(event.endDate).utc();
  if (endDate.isBefore(moment.utc())) {
    return res.status(423).json({ general: 'This event has already finished' });
  }
