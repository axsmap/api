...
module.exports = async (req, res, next) => {
  let verified;
  try {
    verified = verifySalesforceWebhook(req.headers);
  } catch (error) {
    return next(error);
  }
  if (!verified) {
    return res.status(401).json({ general: 'Invalid Salesforce secret' });
  }
  ...
  try {
    ...
    if (error.code === 'PLEDGE_ALREADY_PAID') {
